...
func initPublicKey(publicKeyString string, publicKeyUri string, s3_client *s3.S3) (*rsa.PublicKey, error) {
	...
	publicKeyReader, _, err := grw.ReadFromResource(publicKeyUri, "", 4096, s3_client)
	if err != nil {
		return nil, errors.Wrapf(err, "error opening public key at uri %q", publicKeyUri)
	}

	publicKeyBytes, err := publicKeyReader.ReadAllAndClose()
	if err != nil {
		return nil, errors.Wrapf(err, "error reading public key at uri %q", publicKeyUri)
	}

	publicKey, err := jwt.ParseRSAPublicKeyFromPEM(publicKeyBytes)
	if err != nil {
		return nil, errors.Wrapf(err, "error parsing RSA public key at uri %q", publicKeyUri)
	}
...
func initPrivateKey(privateKeyString string, privateKeyUri string, s3_client *s3.S3) (*rsa.PrivateKey, error) {
	...
	privateKeyReader, _, err := grw.ReadFromResource(privateKeyUri, "", 4096, s3_client)
	if err != nil {
		return nil, errors.Wrapf(err, "error opening private key at uri %q", privateKeyUri)
	}

	privateKeyBytes, err := privateKeyReader.ReadAllAndClose()
	if err != nil {
		return nil, errors.Wrapf(err, "error reading private key at uri %q", privateKeyUri)
	}

	privateKey, err := jwt.ParseRSAPrivateKeyFromPEM(privateKeyBytes)
	if err != nil {
		return nil, errors.Wrapf(err, "error parsing RSA private key at uri %q", privateKeyUri)
	}
...
func serveFunction(cmd *cobra.Command, args []string) {
	...
	logger := gsl.NewLogger(
		...
		[]gsl.Writer{infoWriter, errorWriter},
...
func init() {
	...
	// Coconut
	serveCmd.Flags().String("coconut-api-url", "http://localhost:8080/", "The API_URL for Coconut.")
	serveCmd.Flags().String("coconut-baselayer-url", "https://{a-c}.tile.openstreetmap.org/{z}/{x}/{y}.png", "The BASELAYER_URL for Coconut.")
	serveCmd.Flags().String("coconut-bundle-url", "https://coconut.spatialcurrent.io/bundle.js", "The url to the bundle.js for Coconut.")
