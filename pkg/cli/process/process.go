		...
		for line := range outputLines {
			...
			*/
			...
			if _, ok := outputPathBuffers[line.Path]; !ok {
				...
				outputWriter, outputBuffer, err := grw.WriteBytes(output.Compression, grw.NoDict)
				if err != nil {
					panic(err)
				}
	...
	go func() {
		for object := range objects {
			...
			*/
			...
			line, err := stream.FormatObject(object, output.Format, output.Header, output.LineSeparator)
			if err != nil {
...
func handleOutput(output *config.Output, outputVars map[string]interface{}, objects chan interface{}, fileDescriptorLimit int, wg *sync.WaitGroup, s3Client *s3.S3, stringer stringify.Stringer, logger *gsl.Logger, verbose bool) error {
	...
	if outputDevice := os.OpenDevice(output.Uri); outputDevice != nil {
		...
		b := pipe.NewBuilder().
			...
			Transform(func(inputObject interface{}) (interface{}, error) {
				b, err := stream.FormatObject(inputObject, output.Format, output.Header, output.LineSeparator)
				if err != nil {
					return "", err
				}
				return string(b), nil
		...
		b = b.OutputF(func(object interface{}) error {
			_, err := fmt.Fprintln(outputDevice, object)
			return err
		})
	...
	go func() {
		...
		for line := range outputLines {
			...
			go func(w *sync.WaitGroup, line struct {
				...
				if output.Mkdirs {
					err = os.MkdirAll(filepath.Dir(line.Path), 0770)
					if err != nil {
...
func processSinkToStream(inputReader io.ByteReadCloser, processConfig *config.Process, s3Client *s3.S3, stringer stringify.Stringer, logger *gsl.Logger) error {
...
func processStreamToStream(inputReader io.ByteReadCloser, processConfig *config.Process, dflVars map[string]interface{}, dflNode dfl.Node, s3Client *s3.S3, logger *gsl.Logger) error {
	if processConfig.Verbose {
	...
	switch processConfig.Input.Type {
	case "geojson.Feature":
		iteratorInput.Type = geojson.TypeFeature
	case "geojson.Geometry":
		iteratorInput.Type = geojson.TypeGeometry
	case "geojson.Point":
		iteratorInput.Type = geojson.TypePoint
	}
...
func processAsStream(inputReader io.ByteReadCloser, processConfig *config.Process, athenaClient *athena.Athena, s3Client *s3.S3, stringer stringify.Stringer, logger *gsl.Logger) error {
	...
	if !(processConfig.Output.CanStream()) {
		return fmt.Errorf("output format %q is not compatible with streaming", processConfig.Output.Format)
	}
	...
	if !stream.CanStream(processConfig.Input.Format, processConfig.Output.Format, processConfig.Output.Sorted) {
...
/*
func processAsBatchOld(inputReader io.ByteReadCloser, processConfig *config.Process, athenaClient *athena.Athena, s3Client *s3.S3, logger *gsl.Logger) error {
...
func processFunction(cmd *cobra.Command, args []string) error {
	...
	if !processConfig.Input.IsAthenaStoredQuery() {
		r, inputMetadata, err := grw.ReadFromResource(&grw.ReadFromResourceInput{
