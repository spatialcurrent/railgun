...
func (h *ServiceHandler) Get(w http.ResponseWriter, r *http.Request, format string) ([]byte, error) {
	vars := mux.Vars(r)
	name, ok := vars["name"]
	if !ok {
		return make([]byte, 0), errors.New("missing service name")
	}
	p, ok := h.Config.GetService(name)
	if !ok {
		return make([]byte, 0), errors.New("no service with name " + name)
	}
...
func (h *ServiceHandler) Delete(w http.ResponseWriter, r *http.Request, format string) ([]byte, error) {
	vars := mux.Vars(r)
	name, ok := vars["name"]
	if !ok {
		return make([]byte, 0), errors.New("missing service name")
	...
	if err != nil {
		return make([]byte, 0), errors.Wrap(err, "error deleting service")
	}
