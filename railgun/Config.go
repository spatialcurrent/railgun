...
func (c *Config) ReloadLayers() error {
	...
	for _, str := range layers {
		...
		title := gtg.TryGetString(m, "title", "")
		description := gtg.TryGetString(m, "description", "")
		datastore, found := c.GetDataStore(gtg.TryGetString(m, "datastore", ""))
		if !found {
			return &railgunerrors.ErrInvalidConfig{Name: "layer", Value: str}
		}
		extent := make([]float64, 0)
		if arr := gtg.TryGet(m, "extent", nil); arr != nil {
			e, err := af.ToFloat64Array.ValidateRun([]interface{}{arr})
			if err != nil {
				return &railgunerrors.ErrInvalidConfig{Name: "layer", Value: str}
			}
			if len(e.([]float64)) > 0 {
		...
		layersList = append(layersList, lyr)
		layersByName[lyr.Name] = lyr
	}
...
func (c *Config) ParseWorkspace(obj interface{}) (Workspace, error) {
	...
	if len(name) == 0 {
		return Workspace{}, &railgunerrors.ErrInvalidConfig{Name: "datastore", Value: obj}
	}
...
func (c *Config) DeleteWorkspace(name string) error {
	if _, ok := c.workspacesByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "workspace", Name: name}
	}
	for _, ds := range c.ListDataStores() {
		if ds.Workspace.Name == name {
			return errors.New(fmt.Sprintf("cannot delete: existing data store % is dependent on workspace %s", ds.Name, name))
		}
	}
...
func (c *Config) ParseDataStore(obj interface{}) (DataStore, error) {
	...
	if len(uri) == 0 {
		return DataStore{}, errors.Wrap(errors.New("uri is blank"), (&railgunerrors.ErrInvalidConfig{Name: "datastore", Value: obj}).Error())
	}
	...
	if len(name) == 0 {
		return DataStore{}, errors.Wrap(errors.New("name is blank"), (&railgunerrors.ErrInvalidConfig{Name: "datastore", Value: obj}).Error())
	}
	title := gtg.TryGetString(obj, "title", "")
	description := gtg.TryGetString(obj, "description", "")
...
func (c *Config) DeleteDataStore(name string) error {
	if _, ok := c.datastoresByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "datastore", Name: name}
	}
	for _, s := range c.ListServices() {
		if s.DataStore.Name == name {
			return errors.New(fmt.Sprintf("cannot delete: existing service % is dependent on data store %s", s.Name, name))
		}
	}
...
func (c *Config) DeleteLayer(name string) error {
	if _, ok := c.layersByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "layer", Name: name}
	}
...
func (c *Config) DeleteProcess(name string) error {
	if _, ok := c.processesByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "process", Name: name}
	}
	for _, s := range c.ListServices() {
		if s.Process.Name == name {
			return errors.New(fmt.Sprintf("cannot delete: existing service % is dependent on process %s", s.Name, name))
		}
	}
...
func (c *Config) ParseService(obj interface{}) (Service, error) {
	name := gtg.TryGetString(obj, "name", "")
	if len(name) == 0 {
		return Service{}, errors.Wrap(errors.New("name is blank"), (&railgunerrors.ErrInvalidConfig{Name: "service", Value: obj}).Error())
	}
	title := gtg.TryGetString(obj, "title", "")
	description := gtg.TryGetString(obj, "description", "")
...
func (c *Config) DeleteService(name string) error {
	if _, ok := c.servicesByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "service", Name: name}
	}
	for _, j := range c.ListJobs() {
		if j.Service.Name == name {
			return errors.New(fmt.Sprintf("cannot delete: existing job % is dependent on service %s", j.Name, name))
		}
	}
...
func (c *Config) ParseJob(obj interface{}) (Job, error) {
	name := gtg.TryGetString(obj, "name", "")
	if len(name) == 0 {
		return Job{}, errors.Wrap(errors.New("name is blank"), (&railgunerrors.ErrInvalidConfig{Name: "job", Value: obj}).Error())
	}
	title := gtg.TryGetString(obj, "title", "")
	description := gtg.TryGetString(obj, "description", "")
	service, found := c.GetService(gtg.TryGetString(obj, "service", ""))
	if !found {
		return Job{}, errors.Wrap(errors.New("could not find service with name "+gtg.TryGetString(obj, "service", "")), (&railgunerrors.ErrInvalidConfig{Name: "service", Value: obj}).Error())
	}
...
func (c *Config) DeleteJob(name string) error {
	if _, ok := c.jobsByName[name]; !ok {
		return &railgunerrors.ErrMissing{Type: "job", Name: name}
	}
